...
class Post_Controller extends \WP_REST_Posts_Controller {
	 ...
	 * This post type has only two statuses: publish, and private.
	 * @since 1.0.0
	 * @access public
	...
	public function get_item_schema(){
		...
		$schema['properties']['status']['enum'] = array('publish', 'private');
		return $schema;
	}
	...
	public function check_read_permission($post){
		...
		if (empty($this->check_is_post_type_allowed($post_type))){
			return false;
		}

		if (absint($post->post_author) === get_current_user_id() || current_user_can('read_others_pixmagixs')){
			return true;
		}
